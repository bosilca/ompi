                    ...
                    size_t* length )
{
    ...
    while( 1 ) {
        ...
        if( OPAL_DATATYPE_LOOP == pElem->elem.common.type ) {
            assert( 0 == (pElem->loop.common.flags & OPAL_DATATYPE_FLAG_CONTIGUOUS) );
...
static inline int
opal_convertor_merge_iov_with_cb( struct iovec* iov,
                                  IOVBASE_TYPE* base, size_t len,
                                  opal_convertor_raw_user_callback_t fct,
                                  void* cb_data)
{
    if( 0 != iov->iov_len ) {
        if( (base == ((char*)iov->iov_base + iov->iov_len)) ) {
            iov->iov_len += len;  /* merge with previous iovec */
            return 0;
        }  /* cannot merge, call the user function and update the iovec */
        if( fct( iov->iov_base, iov->iov_len, cb_data ) )
            return 1;  /* stop here and return */
    }
    iov->iov_base = base;
    iov->iov_len = len;
    return 0;
                           ...
                           size_t* length )
{
    ...
    if( OPAL_LIKELY(pConvertor->flags & CONVERTOR_NO_OP) ) {
         ...
         * was moved to a specific position.
         */
        opal_convertor_get_current_pointer( pConvertor, (void**)&iov.iov_base );
        iov.iov_len = pConvertor->local_size - pConvertor->bConverted;
        *length = iov.iov_len;
        
        fct( iov.iov_base, iov.iov_len, cb_data );
    ...
    DO_DEBUG( opal_output( 0, "opal_convertor_raw_parser( %p, {%p, %p}, %"PRIsize_t " )\n",
                           (void*)pConvertor, *(void**)&fct, cb_data, *length ); );
    ...
    if( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
        ...
        if( count_desc != (current->count * current->blocklen) ) {  /* Not the full element description */
            ...
            if( do_now ) {
                ...
                DO_DEBUG( opal_output( 0, "raw 1. iov[%d] = {base %p, length %" PRIsize_t "}\n",
                                       index, (void*)source_base, blength ); );
                opal_convertor_merge_iov_with_cb( &iov,
                                                  (IOVBASE_TYPE *) source_base, blength, fct, cb_data );
    ...
    while( 1 ) {
        while( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
            ...
            for(size_t _i = 0; _i < do_now; _i++ ) {
                ...
                DO_DEBUG( opal_output( 0, "raw 2. iov[%d] = {base %p, length %" PRIsize_t "}\n",
                                       index, (void*)source_base, blength ); );
                if( opal_convertor_merge_iov_with_cb( &iov,
                                                      (IOVBASE_TYPE *) source_base, blength, fct, cb_data ) )
                    break;  /* no more iovec available, bail out */
