...
mca_coll_accelerator_allreduce(ompi_coll_args_t *args, struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    ompi_coll_args_allreduce(&_fwd, sbuf, rbuf, args->dst.info.count, args->dst.info.datatype, args->op);
    rc = s->c_coll.coll_allreduce(&_fwd, comm, s->c_coll.coll_allreduce_module);
    if (NULL != sbuf1) {
        free(sbuf1);
    }
    if (NULL != rbuf1) {
