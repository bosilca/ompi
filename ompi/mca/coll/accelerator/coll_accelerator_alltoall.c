...
mca_coll_accelerator_alltoall(ompi_coll_args_t *args, struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    ompi_coll_args_alltoall(&_fwd, sbuf, args->src.info.count, args->src.info.datatype, rbuf, args->dst.info.count, args->dst.info.datatype);
    rc = s->c_coll.coll_alltoall(&_fwd, comm, s->c_coll.coll_alltoall_module);
    if (rc < 0) {
        goto exit;;
    }
    if (NULL != rbuf1) {
