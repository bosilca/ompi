...
int mca_coll_ucc_scatter(ompi_coll_args_t *args, struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    COLL_UCC_CHECK(mca_coll_ucc_scatter_init_common(args->src.info.buffer, args->src.info.count, args->src.info.datatype, args->dst.info.buffer, args->dst.info.count,
                                                    args->dst.info.datatype, args->root, false, ucc_module, &req,
                                                    NULL));
...
fallback:
    ...
    return ucc_module->previous_scatter(args, comm, ucc_module->previous_scatter_module);
...
fallback:
    ...
    return ucc_module->previous_iscatter(args, comm, request,
...
int mca_coll_ucc_scatter_init(ompi_coll_args_t *args, struct ompi_communicator_t *comm, ompi_info_t *info, ompi_request_t **request, mca_coll_base_module_t *module)
{
    ...
    COLL_UCC_CHECK(mca_coll_ucc_scatter_init_common(args->src.info.buffer, args->src.info.count, args->src.info.datatype, args->dst.info.buffer, args->dst.info.count,
                                                    args->dst.info.datatype, args->root, true, ucc_module, &req,
                                                    coll_req));
    *request = &coll_req->super;
    return OMPI_SUCCESS;
fallback:
    ...
    return ucc_module->previous_scatter_init(args, comm, info, request,
